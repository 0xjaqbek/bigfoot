  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        <h2 className="text-4xl font-bold text-gray-100 mb-4">{t('paymentSummary')}</h2>
      </div>
  ...
  if (loading) {
    return (
      <div className="backdrop-blur-sm bg-blue-100/70 border border-blue-200/50 rounded-xl p-4 mb-8 flex items-center">
        ...
        <span className="text-sm text-blue-800">{t('refreshingFees')}</span>
      </div>
    );
  }
      ...
      <div className="backdrop-blur-sm bg-red-100/70 border border-red-200/50 rounded-xl p-4 mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <span className="text-sm text-red-800">{t('feesError')}: {error}</span>
          </div>
          <button 
            onClick={onRefresh}
  ...
  return (
    <div className="backdrop-blur-sm bg-white/50 border border-gray-200/50 rounded-xl p-6 mb-8 shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-800">{t('networkFeeTitle')}</h3>
        {isStale && (
          <button 
            onClick={onRefresh}
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {Object.entries(fees).map(([speed, feeData]) => (
          ...
          >
            ...
            <div className="text-xs font-medium capitalize">{t(speed)}</div>
      ...
      <div className="text-center p-3 bg-gray-50/70 rounded-lg">
        <div className="text-sm text-gray-600">{t('selectedFee')} ({t(selectedSpeed)})</div>
  ...
  if (loading) {
    return (
      <div className="backdrop-blur-sm bg-blue-100/70 border border-blue-200/50 rounded-xl p-4 mb-6 flex items-center">
        <RefreshCw className="w-5 h-5 text-blue-600 mr-2 animate-spin" />
        <span className="text-sm text-blue-800">{t('refreshingPrices')}</span>
      </div>
    );
  }
    ...
    return (
      <div className="backdrop-blur-sm bg-red-100/70 border border-red-200/50 rounded-xl p-4 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <AlertTriangle className="w-5 h-5 text-red-600 mr-2" />
            <span className="text-sm text-red-800">{t('pricesError')}: {error}</span>
          </div>
          <button 
            onClick={onRefresh}
            className="text-sm text-red-600 hover:text-red-700 font-medium"
  ...
  if (isStale) {
    return (
      <div className="backdrop-blur-sm bg-yellow-100/70 border border-yellow-200/50 rounded-xl p-4 mb-6">
        <div className="flex items-center justify-between">
          <span className="text-sm text-yellow-800">{t('pricesOutdated')}</span>
          <button 
            onClick={onRefresh}
  ...
  return (
    <div className="backdrop-blur-sm bg-white/50 border border-gray-200/50 rounded-2xl p-8 mb-8 shadow-lg">
      <div className="grid grid-cols-2 gap-6 text-center">
        <div>
          <div className="text-sm text-gray-600 mb-1">{t('network')}</div>
          <div className="font-bold text-lg text-gray-800">{blockchain?.name}</div>
        </div>
        <div>
          <div className="text-sm text-gray-600 mb-1">{t('currencyType')}</div>
      ...
      <div className="border-t border-gray-200/50 mt-6 pt-6 text-center">
        <div className="text-sm text-gray-600 mb-2">{t('toPay')}</div>
        {loading ? (
          <div className="text-2xl font-bold text-gray-400 mb-1">{t('loading')}</div>
        ) : cryptoAmount ? (
          <>
          ...
          <div className="text-2xl font-bold text-red-500 mb-1">{t('calculationError')}</div>
        )}
      </div>
    </div>
  );
